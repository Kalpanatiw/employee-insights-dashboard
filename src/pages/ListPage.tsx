...
import { useData, type EmployeeRow } from '../data/DataContext';
      ...
      try {
        ...
        const mapped: EmployeeRow[] = data.map((row, index) => {
          ...
          const salaryRaw =
            ...
            0;
          const salary =
            typeof salaryRaw === 'number' ? salaryRaw : Number.parseFloat(String(salaryRaw)) || 0;
          return { id, name, city, salary, ...row };
        });
        setRows(mapped);
      } catch (err) {
        console.error(err);
    ...
    <section className="page list-page">
      ...
      {rows.length > 0 && (
        <div className="table-wrapper">
          ...
          <VirtualizedTable rows={rows} onRowClick={handleRowClick} />
        </div>
      )}
    </section>
  );
